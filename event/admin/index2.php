...
<?php
	...
	echo '<a href="index.php"><img src="../images/goback.jpg" alt=""></a><a href="logout.php" class="logout" title="Logout">
	...
	echo "<a href='index2.php?month=". $change_month ."&year=". $change_year ."'><div class='previous'> Prev </div></a>";
	echo "<a href='index2.php?month=". $change_month_next ."&year=". $change_year_next ."'><div class='next'> Next </div></a>";
...
/* Previous Month */
	...
	for($i=$final_day; $i<=$last_month_day_count; $i++):
		...
		$count = count_events($i,$change_month,$change_year,$db);
...
for($i=1; $i<= $day_count; $i++):
	...
	$count = count_events($i,$month,$year,$db);
	...
	echo "<div class='event_day'>";
		...
		echo "<div class='event_heading'>" . $day_name . "</div>";
	$sflag=1;
		if($count >= 1 && $sflag==1) // Special
			{
				echo "<div class='event_count'><span class='event'>" . $count . "</span></div><div class='event_number special'>". $i . "</div>";
				$sflag=0;
			} 
		elseif($today == $date) // today
			{echo "<div class='event_number today'>" . $i . "</div>";}
		elseif($day_name == 'Mon' || $day_name == 'Tue' || $day_name == 'Wed' || $day_name == 'Thu' || $day_name == 'Fri' || $day_name == 'Sat' ) // open normally
			{echo "<div class='event_number open'>". $i . "</div>";}
		/*elseif($day_name == 'Sun' && $count < 1 && $sflag == 0) // close
			{echo "<div class='event_number close'>". $i . "</div>";}*/
		else
			{echo "<div class='event_number close'>". $i ."</div>";} // none
...
if ( ($first_day_of_week=='Monday' && $last_day != 7) || ($first_day_of_week=='Sunday' && $last_day != 1) ) :
	...
	for($i=1; $i<=$first_day; $i++): 
		...
		$count = count_events($i,$change_month_next,$change_year_next,$db);
